...
$.fn.slide=function(){
    ...
    $n=$(this).find('.Right');
    if(opts.auto){
        t = setInterval(function(){showAuto();$(".focus-num span.cur").text($(".ban_nav a.on").text());}, opts.time);
    ...
    $p.click(function(){
        showPre();
        $(".focus-num span.cur").text($(".ban_nav a.on").text());
    });
    $n.click(function(){
        showAuto();
        $(".focus-num span.cur").text($(".ban_nav a.on").text());
...
$('.slide').each(function() {
    $(this).slide();
    $(".focus-num span.all").text($(".album").length); //默认初始化轮播图总数量
});
